...
def get_token(username, password):
    ...
    authorize_response = session.get(
        ...
        params={"callbackUri": PARKANIZER_CALLBACK_URI, "emailHint": ""}
    )
    authorize_response.raise_for_status()

    state_properties = re.findall(r'"transId":"(StateProperties=[^"]*)"', authorize_response.text)[0]
    csrf = authorize_response.cookies["x-ms-cpim-csrf"]

    auth_params = {"tx": state_properties, "p": POLICY}

    self_asserted_response = session.post(
        PARKANIZER_LOGIN_URI + "/SelfAsserted",
        params=auth_params,
        data={"request_type": "RESPONSE", "signInName": username},
        headers={"X-CSRF-TOKEN": csrf}
    )
    self_asserted_response.raise_for_status()

    confirm_response = session.get(PARKANIZER_LOGIN_URI + "/api/CombinedSigninAndSignup/confirmed",
    ...
    assert login_response.json()["status"] == "200", login_response.json()["message"]
